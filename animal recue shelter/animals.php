...
<form method="post" onsubmit="return confirmInsert();">
    ...
    <label for="ShelterDate">ShelterDate:</label>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = $connection->prepare("INSERT INTO animals(AnimalID, Name,Species,Age,ShelterDate) VALUES (?, ?, ?, ?,?)");
    $stmt->bind_param("issss", $AnimalID, $Name,$Species,$Age,$ShelterDate);
    // Set parameters and execute
    $AnimalID = $_POST['driver_id'];
    $Name = $_POST['user_id'];
    $Species = $_POST['license_number'];
    $Age = $_POST['car_model'];
    $ShelterDate = $_POST['carmodel'];
...
<body>
    <center><h2>Animals Table</h2></center>
    <table border="3">
        <tr>
            <th>AnimalID</th>
            <th>Name</th>
...
// Prepare SQL query to retrieve all animals
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        $AnimalID = $row['AnimalID']; // Fetch the AnimalID
        echo "<tr>
            <td>" . $row['AnimalID'] . "</td>
            <td>" . $row['Name'] . "</td>
            <td>" . $row['Species'] . "</td>
            <td>" . $row['Age'] . "</td>
            <td>" . $row['ShelterDate'] . "</td>
            <td><a style='padding:4px' href='delete_animals.php?AnimalID=$AnimalID'>Delete</a></td> 
            <td><a style='padding:4px' href='update_animals.php?AnimalID=$AnimalID'>Update</a></td> 
        </tr>";
    }
